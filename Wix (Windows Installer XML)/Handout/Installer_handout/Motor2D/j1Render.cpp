...
bool j1Render::Load(pugi::xml_node& load) {
	...
	LOG("Loading camera position (%i, %i)", load.child("camera").attribute("x").as_int(), load.child("camera").attribute("y").as_int());
...
bool j1Render::Save(pugi::xml_node& save) {
	...
	save.child("camera").attribute("x") = camera.x;
	save.child("camera").attribute("y") = camera.y;
